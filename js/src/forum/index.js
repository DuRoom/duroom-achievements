...
app.initializers.add('duroom-achievements', app => {
  ...
  extend(UserCard.prototype, 'view', function (element) {
    ...
    const here = app.forum.attribute('duroom-achievements.show-user-card');
    ...
    if (here == "1" && element.attrs.className.includes("UserCard--popover")) {
      ...
      if (achievements !== undefined && achievements !== null) {
        ...
        if (list.children.length > 0 && points > 0) {
          list.children.push(m(Tooltip, { text: app.translator.trans("duroom-achievements.forum.achievement_points") }, m("span.Achievement--Points", app.translator.trans(
            "duroom-achievements.forum.achievement_points") + ": ", m("span.Achievement--Points--Number", points))));
        }
  ...
  extend(CommentPost.prototype, 'view', function (comment) {
    ...
    const here = app.forum.attribute('duroom-achievements.show-post-footer');
    ...
    if (here == "1" && !this.attrs.post.data.attributes.isHidden) {
      ...
      if (comment.children[0].children[2].children.length > 0 && points > 0) {
        comment.children[0].children[2].children.push(m(Tooltip, { text: app.translator.trans("duroom-achievements.forum.achievement_points") }, m("span.Achievement--Points", app.translator.trans(
          "duroom-achievements.forum.achievement_points") + ": ", m("span.Achievement--Points--Number", points))));
      }
    }
  });
