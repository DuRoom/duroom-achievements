...
import ExtensionPage from "duroom/components/ExtensionPage";
import LoadingIndicator from "duroom/components/LoadingIndicator";
import icon from "duroom/helpers/icon";
import AchievementModal from "./AchievementModal";
import AchievementUserModal from "./AchievementUserModal";
import saveSettings from "duroom/utils/saveSettings";
import Stream from 'duroom/utils/Stream';
import Switch from 'duroom/components/Switch';
...
function AchievementsItem(achievement) {
  ...
  const name = achievement
    ...
    : app.translator.trans(
      "duroom-achievements.admin.achievements_page.create_achievement_button"
    );
  const rectangle = (achievement ? achievement.rectangle() : "0,0,32,32").split(',')

  var iconName = achievement ? "" : icon("fas fa-plus");

  const addString = app.translator.trans("duroom-achievements.admin.achievements_page.add_delete_users")
...
export default class AchievementsPage extends ExtensionPage {
  oninit(vnode) {
    ...
    this.settingsPrefix = "duroom-achievements";
        ...
        <div className="container">
          <div className="ExtensionsWidget-list Achievements-list">
            <p className="Achievements-list-heading">
              {app.translator.trans(
                "duroom-achievements.admin.achievements_page.list_heading"
              )}
            </p>
          ...
          <p className="Achievements-list-heading">
              {app.translator.trans(
                "duroom-achievements.admin.achievements_page.show"
              )}
            </p>
          {
            m('form', {onsubmit: this.onsubmit.bind(this)},
              m('fieldset', {className: 'Achievements-settings'}, [
                Switch.component(
                  ...
                  app.translator.trans('duroom-achievements.admin.settings.show-post-footer')
                ),
                Switch.component(
                  {
                  ...
                  app.translator.trans('duroom-achievements.admin.settings.show-user-card')
                ),
                Switch.component(
                  {
                  ...
                  app.translator.trans('duroom-achievements.admin.settings.link-left-column')
                ),
                  Button.component({
                    type: 'submit',
                    className: 'Button Button--primary',
                    disabled: !this.changed()
                }, app.translator.trans('duroom-achievements.admin.achievement_modal.submit_button')),
              ])
            )
          }
          <div className="Achievements-footer">
            <p className="Achievements-list-heading">
              {app.translator.trans(
                "duroom-achievements.admin.achievements_page.instructions_header"
              )}
            </p>
            <ul>
              {app.translator.trans(
                "duroom-achievements.admin.achievements_page.instructions_text"
              )}
            </ul>
          </div>
        </div>
